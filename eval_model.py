...
Tarefas:
  - Intrasentence: probabilidade do termo-alvo no lugar de [MASK]. Dois modos,
    controlados por --intra-scoring:
      * simultaneous (padrão, comportamento original): quando o termo-alvo
        ocupa mais de um subtoken, todos são mascarados ao mesmo tempo em um
        único forward pass — cada subtoken é previsto sem ver os demais
        subtokens do próprio termo.
      * pll: mascara um subtoken do termo-alvo por vez, mantendo os demais
        subtokens do próprio termo visíveis (com seus valores verdadeiros),
        igual à PLL já usada na tarefa intersentence. Uniformiza o
        procedimento entre as duas tarefas e evita penalizar termos que se
        fragmentam em mais subtokens (ex.: SentencePiece na Albertina) só por
        não se verem entre si.
...
Uso:
    ...
    # Intrasentence com PLL (mascara um subtoken do alvo por vez):
    python eval_model.py --model <id> --intra-scoring pll ...
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["simultaneous", "pll"],
        help="Como pontuar termos-alvo com mais de um subtoken na tarefa intrasentence. "
             "'simultaneous' reproduz o comportamento original (todos os subtokens mascarados "
             "de uma vez); 'pll' mascara um subtoken por vez (ver Comentário de qualificação "
             "sobre a Seção 3.1.3).",
    )
...
def _build_masked_input(context_pt: str, sentence_pt: str, tokenizer, max_length: int):
    """
    Constrói input com um [MASK] para CADA subtoken real do termo-alvo no lugar
    de BLANK (bug anterior: sempre inserida um único [MASK] via substituição de
    texto, independente do tamanho real do termo-alvo).

    Usa tokenizer(texto, add_special_tokens=True) — o mesmo mecanismo já usado
    em todo o resto do arquivo (inclusive _pll_score) e confirmado como
    portável entre BERT, ModernBERT e DeBERTa-v2 neste ambiente. Evitado de
    propósito: tokenizer.build_inputs_with_special_tokens(), que não está
    disponível em todas as versões/backends do transformers.

    O número de subtokens do termo-alvo é determinado por alinhamento de
    prefixo E sufixo contra a tokenização da sentença completa (mesma técnica
    usada em analyze_subtokens.py) — necessário com SentencePiece
    (Albertina/DeBERTa-v2), cujo marcador ▁ (word boundary) muda conforme o
    fragmento é tokenizado isolado vs. em contexto.
    ...
    if blank_pos == -1:
        # sem BLANK: não haverá posições mascaradas
        enc = tokenizer(ctx, add_special_tokens=True, max_length=max_length,
                         truncation=True, return_tensors="pt")
        return enc["input_ids"], [], []

    prefix_text = ctx[:blank_pos]
    suffix_text = ctx[blank_pos + len("BLANK"):]

    # Tokeniza a sentença completa (já preenchida) e alinha pelo prefixo E
    # pelo sufixo para descobrir exatamente quantos subtokens o termo-alvo
    # ocupa — evita o bug de assumir sempre 1 subtoken.
    full_ids = tokenizer.encode(sent, add_special_tokens=False)
    prefix_ids = tokenizer.encode(prefix_text.rstrip(), add_special_tokens=False) if prefix_text.strip() else []
    suffix_ids = tokenizer.encode(suffix_text.lstrip(), add_special_tokens=False) if suffix_text.strip() else []
    ...
    if frag_len <= 0:
        ...
        frag_len = 1
    frag_ids = full_ids[n_pre: n_pre + frag_len]
    if not frag_ids:
        frag_ids = full_ids[:1] if full_ids else [tokenizer.unk_token_id]
        frag_len = len(frag_ids)

    # Insere frag_len tokens [MASK] separados por espaço no lugar de BLANK —
    # preserva a adjacência original de pontuação (ex.: "muito BLANK!").
    mask_str = " ".join([tokenizer.mask_token] * frag_len)
    masked_text = prefix_text + mask_str + suffix_text
...
def _intrasentence_score_simultaneous(input_ids, mask_positions, target_ids,
                                       model, device: str, vocab_size: int) -> float:
    """
    Escore original: todos os subtokens do termo-alvo são mascarados ao mesmo
    tempo, em um único forward pass. Quando o alvo ocupa mais de um subtoken,
    cada posição é prevista sem ver os demais subtokens do próprio alvo —
    apenas o contexto ao redor. Mantido para reproduzir/comparar com a
    primeira versão dos resultados (ver --intra-scoring).
    """
    with torch.no_grad():
    ...
    return float(np.exp(np.mean(token_log_probs))) if token_log_probs else 0.0


def evaluate_intrasentence(tokenizer, model, dataset: dataloader.StereoSet,
                           device: str, max_length: int, scoring: str = "simultaneous") -> list:
    vocab_size = model.config.vocab_size  # para bounds check
    mask_token_id = tokenizer.mask_token_id
    results = []
    desc = f"Intrasentence ({scoring})"
    for example in tqdm(dataset.get_intrasentence_examples(), desc=desc):
        for sent in example.sentences:
            ...
            if scoring == "pll":
                ...
                token_log_probs = []
                for pos, tid in zip(mask_positions, target_ids):
                    if pos >= seq_len or not (0 <= tid < vocab_size):
                        continue
                    ids_t = torch.tensor([base_ids]).to(device)
                    ids_t[0, pos] = mask_token_id
                    with torch.no_grad():
                        logits = model(ids_t).logits  # (1, seq_len, vocab)
                ...
                score = _intrasentence_score_simultaneous(
                    input_ids, mask_positions, target_ids, model, device, vocab_size
                )
...
def _output_path(model_id: str, intra_scoring: str) -> str:
    slug = model_id.replace("/", "_").replace("-", "_").lower()
    suffix = "_pll" if intra_scoring == "pll" else ""
    return f"predictions/predictions_{slug}{suffix}.json"


def main():
    args = parse_args()
    device = "cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu"
    output = args.output or _output_path(args.model, args.intra_scoring)
    ...
    if not args.skip_intrasentence:
        print(f"\n[Intrasentence] MLM scoring ({args.intra_scoring})...")
        result["intrasentence"] = evaluate_intrasentence(
            tokenizer, model, dataset, device, args.max_length, scoring=args.intra_scoring
        )
